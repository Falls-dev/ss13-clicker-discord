  ...
  >
    <div class="popup d-flex flex-column align-items-center">
      ...
      <div class="d-flex align-items-center mt-1" v-if="item.equipmentSlot">
        ...
        <span class="equippable-name d-md-block text-uppercase ml-1">{{$equipSlot(item.equipmentSlot)}}</span>
      </div>
      <span v-if="item.healAmount" class="mt-1">{{ $t('inventory.heals', { amount: item.healAmount }) }}</span>
      <div
        ...
        :key="'req'+index"
      >
        <span>{{ $t('inventory.requires') }}</span>
      ...
      >{{ $t('inventory.enables', { what: allow.toUpperCase() }) }}</span>
      <span
      ...
      >{{item.stats.moveTime > 0 ? `+${item.stats.moveTime}` : item.stats.moveTime}} {{ $t('inventory.moveTime') }}</span>
      <span
        class="description mt-1"
        v-if="item.stats && item.stats.attackSpeed != undefined"
      >{{ $t('inventory.attackSpeed', { time: item.stats.attackSpeed }) }}</span>
      <span
        class="description mt-1"
        v-if="item.fleeChance"
      >{{ $t('inventory.fleeChance', { chance: item.fleeChance }) }}</span>
